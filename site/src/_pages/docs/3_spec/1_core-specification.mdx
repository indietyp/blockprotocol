...
- MUST have a `detail` object which:
  - MUST include a `requestId`, which MUST be a `uuid`
    - if the message is a [direct response](#processing-messages) to another message, it MUST provide the `requestId` of the original message
  ...
  - MUST contain one or both of `data` and `errors`
    ...
    - `errors` MUST be an array of objects, each of which MUST have a `code` and `message` – both strings – and MAY have `extensions`, an arbitrary JSON object defined by the service specification. Valid error codes are defined per message, in an `errorCodes` property in [the message definition](#messages-error-codes), in addition to error codes valid for ALL messages defined in the [processing messages](#processing-messages) section of this specification.
...
#### <Hidden>Processing messages </Hidden>Overview

Messages can be sent from applications to blocks, or from blocks to applications.

Each should process any valid message sent to them according to the functionality described in the relevant specification.

Where messages expect a response, one should be provided.

Embedding applications and blocks need not implement logic for handling every service, or every message in every service.

#### <Hidden>Processing messages </Hidden>Specification

Where an embedding application or a block processes a particular message, they MUST process it in a way
which is consistent with the description of its intended functionality in the relevant service specification.

Where the specification for a received message includes a `respondedToBy`, the receiver MUST send a subsequent message
with a `name` matching the value for `respondedToBy`, with a `requestId` matching that of the original message.

If a block or an embedding application receives a message with a `service` that they do not recognise or do not implement,
they MUST send a message in response with an error `code` of `NOT_IMPLEMENTED`. The `name` in the response message MUST
match the value for `respondedToBy` in the original message. If `respondedToBy` in the original message is undefined,
the original message's `name` MUST be used. In either case the `requestId` of the original message should be used.

Except where a message is a response to another message as described in the above two paragraphs
– i.e. responding to a message which is specified as having a `respondedToBy`, or returning a `NOT_IMPLEMENTED` code
– the `requestId` MUST be freshly generated for each message.

If a block or an embedding application receives a message with a `service` that they do implement, but a message `name`
that they do not, they SHOULD ignore the message.

For completeness, the following messages also SHOULD be ignored:

- messages with a `name` that is not listed in the relevant `service` specification
- messages that match the source of the receiver, i.e. a block SHOULD ignore messages with `source: "block"`
...
A service specification MUST include any messages to be sent as part of the service under a `messages` key on the JSON file defining the service.
...
- <span id="messages-error-codes">`errorCodes`</span>: valid codes for any
  errors encountered in producing the message data (error codes defined in the
  core specification SHOULD NOT be repeated here – their validity is not
  message-specific but condition-specific as described
  [here](#processing-messages)). this may appear alongside or instead of `data`
